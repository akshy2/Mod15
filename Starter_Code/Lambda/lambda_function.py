...
def recommend_portfolio(intent_request):
    ...
    if (age is None) or (investment_amount is None) or investment_amount == '' or age == '':
        return build_validation_result(True, None, None)

    # YOUR CODE GOES HERE!
    if int(age)>65 or int(age)<0:
        return build_validation_result(
                False,
                "age",
                "Please enter a valid age younger than 65 years old to use this service.",
            )
    elif int(investment_amount)<5000:
        return build_validation_result(
                False,
                "investmentAmount",
                "please provide an investment greater than 5000.",
            )
    else:
        if risk_level == "None":
             return close(
